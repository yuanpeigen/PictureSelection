package com.czechyuan.imagepicker.ui;
...
import com.czechyuan.imageedit.EditImageActivity;
import com.czechyuan.imageedit.GraffitiParams;
...
import com.czechyuan.imagepicker.view.SuperCheckBox;
...
public class ImagePreviewActivity extends ImagePreviewBaseActivity implements ImagePicker.OnImageSelectedListener, View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.btn_ok) {
            ...
            params.mPaintSize = 16;
            // 图片路径
            params.mImagePath = mImageItems.get(mCurrentPosition).path;
            EditImageActivity.startActivityForResult(ImagePreviewActivity.this, params, REQ_CODE_GRAFFITI);
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQ_CODE_GRAFFITI) {
            ...
            if (resultCode == EditImageActivity.RESULT_OK) {
                String path = data.getStringExtra(EditImageActivity.KEY_IMAGE_PATH);
                if (TextUtils.isEmpty(path)) {
                    return;
                }
                //发送广播通知图片增加了
                ImagePicker.galleryAddPic(this, new File(path));
                ImageItem imageItem = mImageItems.get(mCurrentPosition);
                if (imagePicker.isSelect(imageItem)) {
                    int index = imagePicker.getSelectedImages().indexOf(imageItem);
                    imagePicker.getSelectedImages().get(index).path = path;
                }
                imageItem.path = path;
